...
export function OrderList({ tableNumber }: OrderProps) {
  const { orders, subtotal, total, totalDiscount } = useOrder()

  // Filtrar os pedidos que correspondem ao número da mesa
  const filteredOrders = Object.entries(orders).filter(
    ([key, value]) => key === tableNumber,
  )
      ...
      <div className="flex h-full w-full flex-col gap-[15px] rounded-[10px] bg-LightGray px-[10px] pt-[4px] shadow">
        <div className="flex w-full flex-row justify-between gap-[15px]">
          <div className="flex w-full flex-col items-center justify-between">
            <h1 className="font-medium text-black">Pedido #{tableNumber}</h1>
          </div>
          <div>
            <Button className="h-[25px] w-[100px] bg-white text-primary shadow hover:bg-primary hover:text-white">
              Limpar Tudo
            </Button>
          </div>
        </div>
